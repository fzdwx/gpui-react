...
var ReactReconciler = __toESM(require_react_reconciler(), 1);
...
var libName = `libgpui_renderer.${suffix}`;
var libPath = join(import.meta.dir, "../../rust/target/release", libName);
...
function renderFrame(element) {
  ...
  const typeBuffer = new TextEncoder().encode(element.type + "\x00");
  liveBuffers.push(typeBuffer.buffer);
  const typePtr = ptr(typeBuffer);
  const textContent = element.text || " ";
  const textBuffer = new TextEncoder().encode(textContent + "\x00");
  ...
  lib.symbols.gpui_render_frame(globalIdPtr, typePtr, textPtr, childCountPtr, childrenPtr, resultBuffer);
  const status = new DataView(resultBuffer.buffer).getInt32(0, true);
  if (status !== 0) {
...
function batchElementUpdates(elements) {
  ...
  const elementsBuffer = new TextEncoder().encode(elementsJsonString + "\x00");
...
function mapStyleToProps(props) {
  const result = {};
  if (props.color) {
...
var config = {
  ...
  createInstance(type, props) {
    ...
    const styles = mapStyleToProps(styleProps);
    const eventHandlers2 = extractEventHandlers(props);
    const element = { ...styles, eventHandlers: eventHandlers2 };
    const id = elementStore.createElement(type, undefined, element);
    console.log("createInstance:", { type, id, styles, eventHandlers: eventHandlers2 });
    queueElementUpdate(elementStore.getElement(id));
    return id;
  },
...
var reconciler = ReactReconciler.default(config);

// src/renderer/index.ts
function createRoot() {
  init(800, 600);
  createWindow(800, 600);
  const rootContainer = reconciler.createContainer(1, null, false, null, "", () => {}, null);
  return {
    render(children) {
      reconciler.updateContainer(children, rootContainer, null, null, () => {
