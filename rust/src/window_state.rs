...
impl WindowState {
    ...
    pub fn rebuild_tree(&self, root_id: u64, children: &[u64]) {
        ...
        if let Some(root) = element_map.get(&root_id) {
            ...
            let mut element_map = self
                .element_map
                .lock()
                .expect("Failed to acquire element_map lock (second)");
    ...
    pub fn update_element_tree(&self) {
        let mut tree = self
            .element_tree
            .lock()
            .expect("Failed to acquire element_tree lock");
        ...
        if let Some(root) = element_map.get(&root_id) {
            ...
            fn update_children(
                ...
                let children_ids: Vec<u64> = element
                    .children
                    .iter()
                    .filter_map(|c| Some(c.global_id))
                    .collect();
