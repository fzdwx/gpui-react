...
use gpui::{
    App, Bounds, DispatchPhase, Element, ElementId, GlobalElementId,
    HitboxBehavior, InputHandler, IntoElement, LayoutId, Modifiers,
    MouseDownEvent, Pixels, Point, SharedString, Size, TextRun,
    UTF16Selection, Window, fill, px, rgb,
};

use super::{ElementStyle, ReactElement};
use super::input::{
    InputMode, RopeExt, Selection,
    CURSOR_WIDTH,
};
use crate::event_types::{EventData, FocusEventData, InputEventData};
use crate::focus;
...
impl RootInputHandler {
    ...
    fn get_focused_input_state(&self) -> Option<InputStateRef> {
        ...
        let element_map = window
            ...
            .element_map
            .lock()
            .expect("Failed to acquire element_map lock");
        ...
        let input_states = window
            .state()
            .input_states
            .lock()
            .expect("Failed to acquire input_states lock");
...
impl InputHandler for RootInputHandler {
    ...
    fn replace_text_in_range(
        ...
        log::debug!(
            "[RootInputHandler] replace_text_in_range: range={:?}, text={:?}",
            range,
            text
        );
        ...
        let value = {
            let mut input_states = gpui_window
                .state()
                .input_states
                .lock()
                .expect("Failed to acquire input_states lock");
            ...
            let state = input_states.entry(focused_id).or_insert_with(|| {
                ...
                InputStateRef {
                    ...
                    selection: Selection::cursor(element.style.value.as_ref().map(|v| v.len()).unwrap_or(0)),
                    marked_range: None,
                    cursor_visible: true,
            ...
            let range = range
                ...
                .unwrap_or_else(|| state.selection.range());
...
impl RootInputHandler {
    fn dispatch_input_event(&self, state: &InputStateRef, input_type: &str) {
...
/// Reference to input state that can be passed around.
#[derive(Clone)]
pub struct InputStateRef {
    pub element_id: u64,
    pub text: Rope,
...
impl InputStateRef {
    ...
    pub fn selected_text_range(&self) -> Range<usize> {
    ...
    pub fn unmark_text(&mut self, _window: &mut Window, _cx: &mut App) {
...
impl Element for ReactInputElement {
    ...
    fn id(&self) -> Option<ElementId> {
        Some(ElementId::from(self.element.global_id as usize))
    }
    ...
    fn prepaint(
        ...
        let state = input_states.entry(element_id).or_insert_with(|| {
            InputStateRef {
                element_id,
                text: Rope::from(self.element.style.value.as_deref().unwrap_or("")),
                selection: Selection::cursor(
                    self.element.style.value.as_ref().map(|v| v.len()).unwrap_or(0),
                ),
                marked_range: None,
                cursor_visible: true,
                mode: if self.element.style.multi_line == Some(true) {
                    InputMode::multi_line()
                } else {
                    InputMode::plain_text()
                },
                masked: self.element.style.input_type.as_deref() == Some("password"),
                placeholder: self.element.style.placeholder.clone().unwrap_or_default(),
                disabled: self.element.style.disabled.unwrap_or(false),
            }
        });
        ...
        // Register mouse click event for focus
        {
            ...
            window.on_mouse_event(move |_event: &MouseDownEvent, phase, window, _cx| {
                if phase == DispatchPhase::Bubble && hitbox.is_hovered(window) {
                    log::debug!(
                        "[Input] MouseDown: window_id={}, element_id={}",
                        window_id,
                        element_id
                    );
