...
impl InputMode {
	...
	pub fn is_multi_line(&self) -> bool { !self.is_single_line() }

	/// Check if this is auto-grow mode.
	pub fn is_auto_grow(&self) -> bool { matches!(self, Self::AutoGrow { .. }) }

	/// Check if this is code editor mode.
	pub fn is_code_editor(&self) -> bool { matches!(self, Self::CodeEditor { .. }) }
	...
	pub fn set_rows(&mut self, new_rows: usize) {
		match self {
			...
			Self::AutoGrow { rows, min_rows, max_rows, .. } => {
