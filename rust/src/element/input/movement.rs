...
pub fn move_to_start(_text: &Rope, _selection: &Selection) -> Selection { Selection::cursor(0) }
...
pub fn move_to_prev_word(text: &Rope, selection: &Selection) -> Selection {
	let mut offset = if selection.is_empty() { selection.end } else { selection.ordered().0 };
...
pub fn move_to_next_word(text: &Rope, selection: &Selection) -> Selection {
	let mut offset = if selection.is_empty() { selection.end } else { selection.ordered().1 };
...
/// Select all text.
pub fn select_all(text: &Rope) -> Selection { Selection::new(0, text.len_bytes()) }
