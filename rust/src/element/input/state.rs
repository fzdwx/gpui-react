use gpui::{
    App, Bounds, ClipboardItem, EventEmitter, FocusHandle, MouseButton,
    MouseDownEvent, MouseMoveEvent, MouseUpEvent, Pixels, Point, ScrollHandle, Size, Window, px,
};
use ropey::Rope;
use std::ops::Range;

use super::{
    movement, select_word, BlinkCursor, Change, History, InputMode, LastLayout, RopeExt,
    Selection, TextPoint, TextWrapper,
};
...
impl InputState {
    ...
    pub fn cursor_position(&self) -> TextPoint {
        self.text.offset_to_point(self.cursor())
    }
    ...
    pub fn replace_text(
        ...
        // Update text wrapper
        self.text_wrapper
            .update(&self.text, &range, &Rope::from(new_text));

        // Record change
        self.history
            .push(Change::new(range, old_text, new_text.to_string()));
    ...
    pub fn replace_ime(
        ...
        self.ime_marked_range = if text.is_empty() {
            None
        } else {
