...
async function createRelease() {
    ...
    console.log(
        `   Create GitHub Release at: https://github.com/fzdwx/gpui-react/releases/new?tag=v${newVersion}\n`
    );
}
...
function showStatus() {
    ...
    try {
        const remoteTracking = execSync("git rev-parse --abbrev-ref @{u}", {
            cwd: workspacePath,
            encoding: "utf8",
        })
            .toString()
        ...
        const remoteSha = execSync(`git rev-parse ${remoteTracking}`, {
            cwd: workspacePath,
            encoding: "utf8",
        })
            .toString()
        ...
        if (localSha !== remoteSha) {
            console.log(
                `⚠️  Local (${localSha.substring(0, 7)}) differs from remote (${remoteSha.substring(0, 7)})`
            );
        } else {
