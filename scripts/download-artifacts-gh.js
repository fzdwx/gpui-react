...
async function downloadArtifact(runId, artifactName, outputDir) {
  ...
  try {
    ...
    await execAsync(`gh run download ${runId} --name ${artifactName} --dir "${tempDir}/${artifactName}"`);
    ...
    console.warn(`[download-artifacts] Warning: Failed to download ${artifactName}: ${error.message}`);
  }
}
...
async function main() {
  try {
    ...
    console.log(`[download-artifacts] Found run: ${workflowRun.displayTitle} (${workflowRun.headSha.substring(0, 7)})`);
